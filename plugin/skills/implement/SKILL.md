...
4. **Build it test-first** with the loop below, at the pre-agreed seams. Run the typechecker (if the project has one) and single test files regularly. The full suite runs once, at the end: `"${CLAUDE_PLUGIN_ROOT}/bin/supermatt" test` when the repo is opted in (it remembers which files passed, so the commit, the review and the later stages do not run the suite again on the same files), otherwise the project's own test command.
5. **Commit** to the current branch. When the `tests_before_commit` rule is on, the commit runs the configured test command first unless these files already passed it, and a red suite blocks it: fix the code, not the rule.
