...
Give each ticket its **blocking edges**: the other tickets that must complete before it can start. A ticket with no blockers can start immediately. A ticket that depends on a decision only the user can make (wording, a price, an address) gets the `needs-info` label instead of `ready-for-agent`, with the open question written in the ticket. The pipeline skips it until the user answers.
...
Publish the approved tickets. **How** depends on the tracker `/supermatt:setup` configured; the tickets are the same either way, only the shape of the blocking edges changes:
