...
The diagram sources are the JSON specs in `docs/diagrams/`. Each has one dark-theme SVG in `docs/images/`, so it stays sharp at any size. To change a diagram, edit its spec, validate and render it with Archify (`validate <type> <name>.json --quality showcase`, then `render`; the rendered HTML is gitignored), and use the HTML's Export menu to save the SVG. Before committing the SVG, remove its `@font-face` rules and its `@media (prefers-color-scheme: light)` block (GitHub blocks embedded fonts, and the docs use the dark theme only), and set its `viewBox` to the content you want shown.
